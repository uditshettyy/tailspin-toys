...
The homepage includes category and publisher filters so shoppers can narrow the game catalog while keeping the result set accessible and keyboard-friendly.
...
Repository coding standards are maintained in the [Copilot instructions](.github/copilot-instructions.md) and the scoped [instruction files](.github/instructions/). In particular:

- Comments explain intent, constraints, and non-obvious decisions — not code that is already self-explanatory.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc for their purpose, parameters, and return values.
- Reusable Astro components document their `Props` interface and component contract.
- TypeScript uses two-space indentation, single-quoted strings, semicolons, trailing commas in multiline structures, and spaces inside object braces; ESLint enforces the applicable formatting rules.
