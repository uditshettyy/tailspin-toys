...
- Define and document a `Props` interface in frontmatter for every reusable component. Each property should state its purpose and any important constraints.
...
- Add comments only for intent, accessibility reasoning, or non-obvious decisions; do not restate the surrounding markup or Astro syntax.
- Keep component documentation current when props, slots, or rendered behavior change.
